...
- "암진단비 특약 5천만원으로 넣어줘", "정기특약 빼줘", "암보험 취소해줘" 같은 대화형 요청을 인식해 설계 상태를 실시간 갱신
- 암보험 + 연금보험처럼 **서로 다른 상품을 동시에 여러 개** 가입하는 것도 지원 (주계약은 리스트 구조)
...
> 단일 앱 구조로는 구현할 수 없습니다. 음성 인터페이스(STT/TTS)도 검토는
> 했으나, 핵심기능과 무관한 스코프라 이번 범위에서는 제외했습니다.
...
- 앱이 외부 API·secrets에 접근하는 모든 지점(챗 모델 초기화, 벡터스토어 로드, 약관 검색, 답변 생성, 추천 계산)에 예외 처리를 두어, 쿼터 초과나 설정 누락 같은 실패가 나도 Streamlit 기본 에러 화면 대신 원인이 보이는 안내 메시지로 낙하합니다.
- 예를 들어 `GOOGLE_API_KEY`가 아예 없으면 앱 전체가 죽는 대신 "API 키가 설정되지 않았습니다"라는 명확한 메시지와 함께 안전하게 멈추고, 검색이 실패해도 "확인이 필요합니다"로만 대응할 뿐 대화 자체는 계속됩니다.

### 응답 캐싱

- 같은 질문이 반복되면(데모 리허설, 재질문 등) 검색·답변 생성 API를 다시 부르지 않고 캐시된 결과를 그대로 반환합니다 — 응답 속도가 빨라지고 무료 등급 쿼터도 아낄 수 있습니다.
- 답변 캐시의 키에는 질문뿐 아니라 그 순간의 검색 근거·가입설계 상태까지 전부 포함시켜서, 근거나 설계가 조금이라도 바뀌면 캐시가 자동으로 무효화됩니다 — 오래된 답이 잘못 재사용될 위험이 없습니다.
